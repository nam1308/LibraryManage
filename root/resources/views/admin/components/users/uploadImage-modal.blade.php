...
<div class="w-100 uploadFile position-relative">
    <div class="wrapper {{ isset($data->avatar) ? 'active' : '' }}">
        <div class="image img img_block">
            <img src="{{ isset($data->avatar) ? asset("storage/$data->avatar") : '' }}" alt="" id="imgEditModal" class="img-thumbnail rounded-circle">
        </div>
    </div>
    ...
    <div class="w-100 uploadFile_content image-none">
        ...
        <input class="ipt-avatar" type="file" name="avatar" value="{{ isset($data) ? $data->avatar : '' }}" hidden>
    </div>
</div>
