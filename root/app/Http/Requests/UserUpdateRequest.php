...
class UserUpdateRequest extends FormRequest
{
    ...
    {
        return [
            'employee_id' => ['required', 'max:30', Rule::unique('users', 'employee_id')->ignore($this->input('id'))->whereNull('deleted_at')],
            'email'=> ['required', 'max:50', 'email', Rule::unique('users', 'email')->ignore($this->input('id'))->whereNull('deleted_at'), config('constants.EMAIL_REGEX')],
